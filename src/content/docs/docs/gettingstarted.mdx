...
1. Run `sudo xcode-select --select /Applications/Xcode.app/Contents/Developer` (This will ensure the `xcodebuild` CLI tool works normally.)
...
1. Create and launch an Android emulator (which is the Android equivalent of the iPhone Simulator and is used for local app development). The `skip` tool has a command to quickly install and configure an Android emulator for your project:
